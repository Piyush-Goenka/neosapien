...
7. **Device test matrix is "1 real + 1 emulator"**: real iPhone + Android emulator on Mac. Brief explicitly allows this. OEM-specific behaviors on physical Android devices have not been demonstrated.
8. **Auth is anonymous-only by design** per the brief. No recovery on reinstall — new installation = new identity = new code.
9. **Metered-connection policy is enforced but has no tap-to-confirm UX** — the `confirmOnMetered` policy currently blocks and instructs the user to switch policy on the sender. A proper confirm dialog was out of scope for today.
